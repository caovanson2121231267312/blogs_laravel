<div class="card-body">
    <div class="flex flex-between">
        <div>
            <a href="{{ route('fields.create') }}" class="btn btn-outline-primary" type="button">
    ...
    <div class="table-responsive">
        <table class="mt-3 mb-3 table table-hover table-border">
            <thead>
                <tr>
                    ...
                    <x-th value="Parent_id" data="parent_id" :sortBy="$sortBy" :sortAscending="$sortAscending" />
                    <x-th value="Jobs" data="jobs_count" :sortBy="$sortBy" :sortAscending="$sortAscending" />
                ...
                @else
                    @foreach ($data as $value)
                        <tr wire:key="item-{{ $value->id }}">
                            ...
                            <td>{{ $value->jobs_count }}</td>
                            <td>{{ date_format($value->created_at, 'H:i:s d/m/Y') }}</td>
                            <td>
                                <div class="text-center">
                                    @can('role-create')
                                        <a class="btn btn-success" href="{{ route('fields.show', $value->id) }}"
                                            data-tippy-content="{{ __('messages.detail') }}">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                    @endcan
                                    @can('role-edit')
                                        <a class="btn btn-warning" href="{{ route('fields.edit', $value->id) }}"
        ...
        <x-modal_confirm />
        {{-- <x-modal_reset_confirm url="{{ route('resetfields') }}" /> --}}
