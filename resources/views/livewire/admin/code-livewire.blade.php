<div class="card-body">
    <div class="flex flex-between">
        <div>
            <a href="{{ route('codes.create') }}" class="btn btn-outline-primary" type="button">
    ...
    <div class="table-responsive">
        <table class="mt-3 mb-3 table table-hover table-border">
            <thead>
                <tr>
                    ...
                    <x-th value="Products" data="products_count" :sortBy="$sortBy" :sortAscending="$sortAscending" />
                        ...
                        <tr wire:key="item-{{ $value->id }}">
                            ...
                            <td>
                                <div class="text-center">
                                    @can('role-create')
                                        <a class="btn btn-success" href="{{ route('codes.show', $value->id) }}"
                                            data-tippy-content="{{ __('messages.detail') }}">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                    @endcan
                                    @can('role-edit')
                                        <a class="btn btn-warning" href="{{ route('codes.edit', $value->id) }}"
                                            data-tippy-content="{{ __('messages.edit', ['name' => $value->name]) }}">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                    @endcan
                                    @can('role-delete')
                                        <form action="{{ route('codes.destroy', $value->id) }}"
        ...
        <x-modal_confirm />
        {{-- <x-modal_reset_confirm url="{{ route('resetcodes') }}" /> --}}
